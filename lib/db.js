...
// Discovery database connection for influencer suggestions
export function getDiscoveryDb() {
  if (!process.env.DISCOVERY_DATABASE_URL) {
    console.warn('[Discovery] DISCOVERY_DATABASE_URL not set - discovery features disabled');
    return null;
  }
  return neon(process.env.DISCOVERY_DATABASE_URL);
}

// Get suggested influencers from discovery based on user's campaign profile
export async function getDiscoverySuggestions(userPlatforms, userTags, userRegions, limit = 5) {
  const discoverySql = getDiscoveryDb();
  if (!discoverySql) return [];

  try {
    // Build filters based on user's existing campaign preferences
    const platformFilter = userPlatforms.length > 0
      ? userPlatforms.map(p => p.toUpperCase())
      : ['TWITCH', 'YOUTUBE', 'TIKTOK', 'KICK', 'INSTAGRAM'];

    // Query high-performing streamers that match user's profile
    const suggestions = await discoverySql`
      SELECT
        id,
        platform,
        username,
        ...
        avatar_url,
        igaming_score,
        gambling_compatibility,
        historical_cpa,
        historical_conversions
      FROM discovery_creators
      WHERE platform = ANY(${platformFilter}::text[])
        AND followers > 1000
        AND fraud_check = 'CLEAN'
        AND (historical_cpa IS NULL OR historical_cpa < 50)
      ORDER BY
        CASE WHEN gambling_compatibility = true THEN 0 ELSE 1 END,
        igaming_score DESC NULLS LAST,
    ...
    return suggestions.map(s => ({
      ...
      igamingScore: s.igaming_score,
      gamblingCompatible: s.gambling_compatibility,
      historicalCpa: s.historical_cpa,
      historicalConversions: s.historical_conversions,
    }));
  } catch (e) {
    console.error('[Discovery] Failed to get suggestions:', e.message);
    return [];
  }
}

// Get discovery stats for context
export async function getDiscoveryStats() {
  const discoverySql = getDiscoveryDb();
  if (!discoverySql) return null;

  try {
    const stats = await discoverySql`
      SELECT
        COUNT(*) as total_creators,
        COUNT(*) FILTER (WHERE gambling_compatibility = true) as igaming_compatible,
        ...
        COUNT(*) FILTER (WHERE platform = 'KICK') as kick_count,
        COUNT(*) FILTER (WHERE platform = 'INSTAGRAM') as instagram_count,
        AVG(engagement_rate) FILTER (WHERE engagement_rate > 0) as avg_engagement_rate
      FROM discovery_creators
      WHERE fraud_check = 'CLEAN'
    `;
    return stats[0] || null;
  } catch (e) {
    console.error('[Discovery] Failed to get stats:', e.message);
    return null;
  }
}

// Add aggregate metrics columns to users table
export async function ensureUserMetricsColumns(sql) {
  try {
    await sql`
      ALTER TABLE users
      ...
      ADD COLUMN IF NOT EXISTS avg_cpa DECIMAL(10,2),
      ADD COLUMN IF NOT EXISTS avg_cpc DECIMAL(10,2),
      ADD COLUMN IF NOT EXISTS avg_cpm DECIMAL(10,2),
      ADD COLUMN IF NOT EXISTS metrics_updated_at TIMESTAMP
    `;
  } catch (e) {
    // Columns might already exist or table doesn't exist
    console.log('[DB] ensureUserMetricsColumns:', e.message);
  }
}

// Update aggregate metrics for a specific user
export async function updateUserMetrics(sql, userId) {
  try {
    // Get aggregate data from influencers and creator_metrics for this user
    const aggregates = await sql`
      SELECT
        COUNT(DISTINCT i.id) as total_creators,
        COALESCE(SUM(i.price), 0) as total_spent,
        COALESCE(SUM(COALESCE(i.total_conversions, i.conversions, 0)), 0) as total_conversions,
        COALESCE(SUM(COALESCE(d.total_views, 0)), 0) as total_views,
        COALESCE(SUM(COALESCE(i.clicks, 0)), 0) as total_clicks,
        AVG(NULLIF(cm.cpa, 0)) as avg_cpa,
        AVG(NULLIF(cm.cpc, 0)) as avg_cpc,
        AVG(NULLIF(cm.cpm, 0)) as avg_cpm
      FROM influencers i
      LEFT JOIN creator_metrics cm ON cm.influencer_id = i.id
      LEFT JOIN (
        SELECT influencer_id, SUM(COALESCE(views, 0)) as total_views
        FROM deliverables
        WHERE UPPER(promotion) = 'YES'
        GROUP BY influencer_id
      ) d ON d.influencer_id = i.id
    ...
    if (aggregates[0]) {
      ...
      await sql`
        UPDATE users SET
          ...
          avg_cpa = ${data.avg_cpa || null},
          avg_cpc = ${data.avg_cpc || null},
          avg_cpm = ${data.avg_cpm || null},
          metrics_updated_at = NOW()
        WHERE user_id = ${userId} OR email = ${userId}
      `;
      console.log(`[DB] Updated metrics for user ${userId}`);
    }
  } catch (e) {
    console.error(`[DB] Failed to update metrics for ${userId}:`, e.message);
  }
}

// Update metrics for all users
export async function updateAllUserMetrics(sql) {
  try {
    // Get all unique user_ids from influencers table
    const users = await sql`
      SELECT DISTINCT user_id FROM influencers WHERE user_id IS NOT NULL
    `;

    for (const user of users) {
      await updateUserMetrics(sql, user.user_id);
    }

    console.log(`[DB] Updated metrics for ${users.length} users`);
    return users.length;
  } catch (e) {
    console.error('[DB] Failed to update all user metrics:', e.message);
    return 0;
  }
}
...
export async function getUserData(sql, userId) {
  ...
  const totals = {
    ...
    totalViews: influencers.reduce((sum, i) => sum + (Number(i.total_views) || 0), 0),
  ...
  for (const inf of influencers) {
    ...
    platforms[platform].views += Number(inf.total_views) || 0;
